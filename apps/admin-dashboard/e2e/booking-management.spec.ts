...
test.describe('예약 현황 테스트', () => {
  ...
  test('상태별 카운트 카드 표시', async ({ page }) => {
    ...
    // 상태 텍스트 중 하나 이상 확인
    const hasStats = await page.getByText(/전체|대기|확정|완료|취소/).first().isVisible().catch(() => false);
  ...
  test('골프장 필터 표시', async ({ page }) => {
    ...
    // 전체 옵션이 있는 셀렉트 확인
    const clubSelect = page.locator('select').filter({ hasText: /전체/ });
    await expect(clubSelect).toBeVisible();
  });

  test('검색 필드 동작', async ({ page }) => {
    // 검색 입력 필드 확인
    const searchInput = page.getByPlaceholder(/고객명, 연락처 검색/);
  ...
  test('상태 필터 클릭', async ({ page }) => {
    // 대기 상태 카드 클릭
    const pendingCard = page.locator('div').filter({ hasText: /^대기/ }).first();
