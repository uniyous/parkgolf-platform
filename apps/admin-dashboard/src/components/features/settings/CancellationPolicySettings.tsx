...
import type { CancellationPolicy, ResolvedCancellationPolicy } from '@/types/settings';
import { PolicyInheritanceBadge } from './PolicyInheritanceBadge';

const defaultPolicy: CancellationPolicy = {
  name: '기본 취소 정책',
  code: 'DEFAULT_CANCEL',
  description: '기본 취소 정책입니다.',
  allowUserCancel: true,
  userCancelDeadlineHours: 72,
...
export const CancellationPolicySettings: React.FC = () => {
  ...
  const { data: resolvedPolicy, isLoading } = useCancellationPolicyResolveQuery();
  const createMutation = useCreateCancellationPolicyMutation();
  const updateMutation = useUpdateCancellationPolicyMutation();
  const deleteMutation = useDeleteCancellationPolicyMutation();

  const isSaving = createMutation.isPending || updateMutation.isPending;

  useEffect(() => {
    if (resolvedPolicy) {
      const resolved = resolvedPolicy as ResolvedCancellationPolicy;
      setPolicy(resolved);
  ...
  const hoursToDisplay = (hours: number): string => {
    if (hours >= 24) {
      ...
      return remainingHours === 0 ? `${days}일` : `${days}일 ${remainingHours}시간`;
    }
    return `${hours}시간`;
  };
        ...
        <div className="p-6 space-y-6">
            ...
            {isEditing ? (
              <div className="flex items-center gap-4">
                <select
                  ...
                  className="px-3 py-2 border border-white/15 rounded-lg bg-white/5 text-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                >
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/10">
            <div>
              ...
              <p className="text-sm text-white/50">비활성화 시 상위 정책이 적용됩니다</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
              ...
              <tbody className="divide-y divide-white/10">
                <tr>
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 text-xs bg-green-500/20 text-green-400 rounded">정책 적용</span>
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 text-xs bg-yellow-500/20 text-yellow-400 rounded">부분 환불</span>
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 text-xs bg-red-500/20 text-red-400 rounded">환불 불가</span>
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 text-xs bg-green-500/20 text-green-400 rounded">전액 환불</span>
                  </td>
                </tr>
                <tr>
