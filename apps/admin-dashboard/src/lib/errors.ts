 ...
 * 통합 에러 처리 유틸리티
 *
 * 이 모듈은 프로젝트 전체의 에러 처리를 일관성 있게 관리합니다.
...
import { getErrorMessage, type BffErrorCode } from '@/types/common';
...
export function isValidationError(error: unknown): error is ValidationError {
  return error instanceof ValidationError || (error as any)?.isValidationError === true;
}
