 ...
 *
 * billing-service에 TOSS_TEST_BYPASS=true 가 설정된 dev 환경에서만 동작.
 * paymentKey가 "e2e_test_*" 패턴이면 billing-service가 토스 API 호출을 건너뛰고
 * 합성 응답을 즉시 반환한다 (NODE_ENV=production에서는 자동 비활성).
 *
 * 운영 안전장치
 *   ─ billing-service: TossApiService 생성자에서 NODE_ENV=production이면 무력화
...
/**
 * E2E 전용 paymentKey 생성. billing-service의 우회 분기를 발화시킨다.
 *
