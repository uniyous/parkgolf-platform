...
// InfoRow: manager-console BasicInfoTab 스타일
...
const ClubInfoTab: React.FC<{ club: Club }> = ({ club }) => (
  <div className="space-y-6">
    {/* 기본 정보 + 운영 정보 (2 column, manager-console 동일) */}
    ...
    {/* 위치 정보 카드 (full-width, manager-console 동일) */}
    <div className="bg-white/10 backdrop-blur-xl rounded-lg border border-white/15">
      <div className="px-4 py-3 border-b border-white/10">
        <h4 className="flex items-center text-base font-medium text-white">
      ...
      <div className="p-4">
        {club.latitude && club.longitude ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <p className="text-xs text-white/30 mt-1">manager-console에서 주소를 저장하면 자동으로 계산됩니다.</p>
          </div>
        )}
      </div>
    </div>
...
// ── Club Courses Tab (manager-console CourseManagementTab 스타일, 조회 전용) ──
  ...
  <div className="space-y-6">
    ...
    {club.courses && club.courses.length > 0 ? (
      ...
      <div className="text-center py-12 bg-white/5 rounded-lg">
        ...
        <p className="mt-1 text-sm text-white/50">manager-console에서 코스를 등록해주세요</p>
      </div>
    )}
  </div>
);
