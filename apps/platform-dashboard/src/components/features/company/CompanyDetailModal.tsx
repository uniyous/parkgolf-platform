...
import type { ClubStatus, ClubType } from '@/types/club';
import { useClubsByCompanyQuery, useGamesByCompanyQuery } from '@/hooks/queries';
...
const CLUB_STATUS_STYLES: Record<ClubStatus, { bg: string; label: string }> = {
  ACTIVE: { bg: 'bg-emerald-500/10 text-emerald-400', label: '운영중' },
  INACTIVE: { bg: 'bg-red-500/10 text-red-400', label: '비활성' },
  MAINTENANCE: { bg: 'bg-yellow-500/10 text-yellow-400', label: '점검' },
  SEASONAL_CLOSED: { bg: 'bg-white/5 text-white/50', label: '계절휴장' },
};

const CLUB_TYPE_STYLES: Record<ClubType, { bg: string; label: string }> = {
  PAID: { bg: 'bg-blue-500/10 text-blue-400', label: '유료' },
  FREE: { bg: 'bg-white/5 text-white/60', label: '무료' },
};

type TabId = 'info' | 'clubs' | 'bookings';

const TABS: { id: TabId; label: string }[] = [
  { id: 'info', label: '기본 정보' },
  { id: 'clubs', label: '골프장' },
  { id: 'bookings', label: '예약 현황' },
];
...
export const CompanyDetailModal: React.FC<CompanyDetailModalProps> = ({
  ...
  const { data: clubs = [] } = useClubsByCompanyQuery(company?.id);
  const { data: games = [] } = useGamesByCompanyQuery(company?.id);
  ...
  const kpiCards = [
    { label: '골프장', value: String(clubs.length), icon: '⛳' },
    { label: '라운드', value: String(games.length), icon: '🎯' },
    { label: '예약', value: '-', icon: '📅' },
    { label: '취소', value: '-', icon: '🚫' },
    { label: '환불', value: '-', icon: '💰' },
  ];
        ...
        >
          ...
          <div className="px-8 py-6">
              ...
              <div>
                {clubs.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-white/50 text-sm">등록된 골프장이 없습니다</p>
                  </div>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/10">
                          <th className="text-left py-3 px-3 text-white/60 font-medium">골프장명</th>
                          <th className="text-center py-3 px-3 text-white/60 font-medium">코스</th>
                          <th className="text-center py-3 px-3 text-white/60 font-medium">홀</th>
                          <th className="text-center py-3 px-3 text-white/60 font-medium">유형</th>
                          <th className="text-center py-3 px-3 text-white/60 font-medium">상태</th>
                        </tr>
                      </thead>
                      <tbody>
                        {clubs.map((club) => {
                          const clubStatusStyle = CLUB_STATUS_STYLES[club.status] ?? CLUB_STATUS_STYLES.ACTIVE;
                          const clubTypeStyle = CLUB_TYPE_STYLES[club.clubType] ?? CLUB_TYPE_STYLES.PAID;
                          return (
                            <tr key={club.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                              <td className="py-3 px-3 text-white">{club.name}</td>
                              <td className="py-3 px-3 text-center text-white/80">{club.totalCourses}</td>
                              <td className="py-3 px-3 text-center text-white/80">{club.totalHoles}</td>
                              <td className="py-3 px-3 text-center">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${clubTypeStyle.bg}`}>
                                  {clubTypeStyle.label}
                                </span>
                              </td>
                              <td className="py-3 px-3 text-center">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${clubStatusStyle.bg}`}>
                                  {clubStatusStyle.label}
                                </span>
                              </td>
                            </tr>
                          );
            ...
            {activeTab === 'bookings' && (
              <div className="text-center py-12">
                <div className="text-3xl mb-3">📅</div>
                <p className="text-white/50 text-sm">예약 현황 기능을 준비 중입니다</p>
              </div>
            )}
          </div>
