          ...
          <>
            <div className="px-4 py-3 space-y-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-bold text-white">{data.clubName}</span>
              </div>
              {data.clubAddress && (
                <div className="flex items-center gap-1.5">
                  <MapPin className="w-3 h-3 text-white/40" />
                  <span className="text-xs text-white/40 truncate">{data.clubAddress}</span>
                </div>
              )}
              {data.date && (
                <div className="flex items-center gap-1.5">
                  <Calendar className="w-3 h-3 text-white/40" />
                  <span className="text-xs text-white/40">{formatDateKorean(data.date)}</span>
                </div>
              )}
            </div>
                ...
                {round.slots.map((slot) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={cn(
                        'text-xs',
                        isSelected ? 'font-bold text-emerald-400' : 'font-medium text-white'
                      )}>
                        {slot.time}
                      </span>
                      {slot.availableSpots != null && (
                        <span className={cn(
                          'text-[10px]',
                          isSelected ? 'text-emerald-400/70' : 'text-white/40'
                        )}>
