...
- **Completion**: 96% (as of 2026-02-14)
- **Target Release**: 2026-Q1
- **Recent Milestone**: Hierarchical Policy System & Platform Dashboard
...
|-------|---------|--------------|----------|
| **Presentation** | User Interface | React 19, SwiftUI, Kotlin Compose | Admin Dashboard, Platform Dashboard, User WebApp, iOS App, Android App |
...
| **Server State** | React Query (TanStack) | 5.x | Server State Management |
| **Client State** | Zustand | 5.x | Auth Store |
...
- 게임 관리 (Game, GameTimeSlot, WeeklySchedule)
- 예약 관리 및 모니터링
...
- Client State: Zustand (auth only)
- Routing: React Router 7.6
- UI: Tailwind CSS 4.1.8 + Lucide React
- Build: Vite 6.3 + SWC
...
// Connected Services
- IAM Service (인증/인가, 회원관리, 메뉴)
- Course Service (골프장/게임 데이터)
- Booking Service (예약 관리, 정책 관리)
...
// Data Models
- User: 일반 사용자
- Admin: 관리자 (Company 소속, Role 기반 권한)
- Company: 가맹점 (CompanyType: PLATFORM, ASSOCIATION, FRANCHISE)
- RoleMaster: 역할 정의 (permissions JSON)
- AdminCompany: 관리자-가맹점 매핑
- Friend/FriendRequest: 친구 관계
- RefreshToken: 토큰 관리
- LoginHistory: 로그인 이력
- CompanyMember: 가맹점별 회원 (source: BOOKING/MANUAL/WALK_IN)
- MenuMaster: 동적 메뉴 시스템
- MenuPermission: 메뉴별 필요 권한
- MenuCompanyType: 메뉴별 가맹점 유형 필터
...
// Core Features - Company Members
- 가맹점별 회원 관리 (CompanyMember)
- 예약 시 자동 등록 (BOOKING source)
- 관리자 수동 등록 / 현장 등록
- 회원 검색 (이름/이메일/전화번호)

// Core Features - Menu System
- DB 기반 동적 메뉴 구성
- 권한 기반 메뉴 접근 제어
- 가맹점 유형별 메뉴 필터링
...
// Domain Structure
- Company: 골프장 운영 회사
- Club: 골프장 (실제 장소, 위경도 좌표)
- Course: 코스 (9홀)
- Hole: 홀 상세 정보
- TeeBox: 티박스 (난이도별)
- Game: 18홀 게임 (전반/후반 코스 조합, SlotMode: TEE_TIME/SESSION)
- GameTimeSlot: 게임별 타임슬롯 (날짜/시간, 예약 인원, Optimistic Lock)
- GameWeeklySchedule: 주간 스케줄 (같은 요일 복수 세션 지원)

// Core Features
- 가맹점별 골프장/코스 관리
- 게임 정의 (전반+후반 코스 조합)
- 타임슬롯 자동 생성 (주간 스케줄 기반)
- 근처 골프장 검색 (Haversine 공식)
- Optimistic Locking 기반 동시성 제어

// Message Patterns
- companies.*
- clubs.* / club.findNearby
- courses.*
- holes.*
...
// Data Models - Booking
- Booking: 예약 (Saga 상태 관리)
- BookingHistory: 예약 히스토리
- GameCache: 게임 정보 캐시
- GameTimeSlotCache: 타임슬롯 가용성 캐시
- OutboxEvent: Transactional Outbox Pattern
- IdempotencyKey: 중복 요청 방지
- Refund: 환불 처리 (금액, 사유, PG 트랜잭션)
- UserNoShowRecord: 사용자 노쇼 기록

// Data Models - Policy (계층형 정책 시스템)
- PolicyScope: PLATFORM > COMPANY > CLUB (3단계 상속)
- CancellationPolicy: 취소 정책 (사용자 취소 허용, 마감 시간, 당일 취소)
- RefundPolicy + RefundRateTier: 환불 정책 (시간대별 환불률)
- NoShowPolicy + NoShowPenalty: 노쇼 정책 (단계별 패널티)
- OperatingPolicy: 운영 정책 (운영 시간, 게임 기본값, 성수기/주말 요금률)
...
// Message Patterns (NATS)
- booking.create/get/list/cancel
...
NATS Pattern Examples:
  # IAM
  - auth.login / auth.validate / auth.refresh
  - users.* / friends.* / iam.companyMembers.* / iam.menus.*

  # Course
  - companies.* / clubs.* / courses.* / holes.*
  - games.* / gameTimeSlots.* / gameWeeklySchedules.*
  - slot.reserve / slot.release (Saga)
  - club.findNearby

  # Booking
  - booking.create / booking.cancel / booking.confirmed
  - policy.cancellation.* / policy.refund.* / policy.noshow.* / policy.operating.*

  # External API
  - weather.current / weather.hourly / weather.forecast
  - location.search.* / location.getCoordinates / location.coord2region
  - agent.chat / agent.reset / agent.status
...
- Added Hierarchical Policy System (Platform > Company > Club)
  - CancellationPolicy, RefundPolicy (시간대별 환불률), NoShowPolicy (단계별 패널티), OperatingPolicy
  - 3단계 스코프 상속: PLATFORM → COMPANY → CLUB (resolve 폴백)
- Added Platform Dashboard (플랫폼 관리자 전용)
  - 가맹점 관리, 플랫폼 정책 설정, 관리자/역할 관리
- Added CompanyMember (가맹점별 회원 관리)
  - 예약 자동 등록(BOOKING), 수동 등록(MANUAL), 현장 등록(WALK_IN)
- Added Game/GameTimeSlot system (18홀 게임 정의)
  - 주간 스케줄 같은 요일 복수 세션 지원 (오전/오후)
- Added Dynamic Menu System (MenuMaster, MenuPermission, MenuCompanyType)
- Fixed State Management documentation (Redux → React Query + Zustand)
- Updated NATS patterns with policy/companyMember/menu patterns
- Updated Database Architecture diagrams
