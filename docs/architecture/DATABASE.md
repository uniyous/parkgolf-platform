...
| booking-service | booking_db | 예약, 결제, 정책, 환불/노쇼, 그룹예약/더치페이, Saga 패턴 |
...
flowchart LR
    ...
    Booking -.->|gameId| Game
    Booking -.->|bookingGroupId| BookingGroup
    BookingGroup -.->|chatRoomId| ChatRoom
...
flowchart TB
    ...
    subgraph group["그룹 예약 (더치페이)"]
        BG_Group["BookingGroup"]
...
erDiagram
    ...
    BookingGroup {
        ...
        int totalPrice
        int pricePerPerson
        string settlementStatus "PENDING/PARTIAL/COMPLETED/CANCELLED"
...
> **BookingGroup**: 더치페이 그룹 예약의 최상위 엔티티. 여러 Booking(팀)을 묶고, 전체 정산 상태(settlementStatus)를 관리한다.
> **BookingParticipant**: 팀별 참여자 목록. `@@unique([bookingId, userId])`로 중복 방지. 개별 결제 완료 시 `PAID`로 전이.
...
| SettlementStatus | `PENDING`, `PARTIAL`, `COMPLETED`, `CANCELLED` | 그룹 정산 상태 |
...
| booking_db | 17 | Booking, BookingGroup, BookingParticipant, Refund, CancellationPolicy, OperatingPolicy, OutboxEvent |
