...
| 특정 서비스 로그 | `gcloud logging read 'resource.labels.container_name="billing-service"' --freshness=30m --limit=50` |
...
| 작업 | 명령 |
|---|---|
| 실시간 로그 | `kubectl --context=parkgolf-dev logs -n parkgolf-dev deploy/billing-service -f` |
...
[Log-based Metric]
  Cloud Logging > Logs-based Metrics > Create Metric
    필터: container_name="billing-service" AND jsonPayload.event="payment.failed"
