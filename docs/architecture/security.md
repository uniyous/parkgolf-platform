...
| 서비스 | 허용 도메인 | 특이사항 |
|--------|-----------|---------|
| consumer-bff | `parkgolf-user*.web.app`, `*user.parkgolfmate.com` | `Authorization` 헤더 |
| manager-bff | `parkgolf-manager*.web.app`, `*admin.parkgolfmate.com`, `*platform.parkgolfmate.com` | `X-Company-Id` 커스텀 헤더 포함 |
...
> 소스: `services/consumer-bff/src/main.ts`, `services/manager-bff/src/main.ts`
...
```typescript
// 글로벌 Rate Limiting (consumer-bff, manager-bff)
...
> 소스: `services/billing-service/src/payment/controller/webhook.controller.ts`
...
| marketplace-saga-service | `marketplace_saga_db` | Saga 오케스트레이션, Step 이력 |
...
> 소스: `services/consumer-bff/src/common/nats/nats.config.ts`
