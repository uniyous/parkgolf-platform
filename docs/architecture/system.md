...
- **자체 플랫폼 시**: booking-service + club-service로 예약 직접 처리
...
| **marketplace-saga-service** | 8080 | marketplace_saga_db | 분산 트랜잭션 Orchestrator, 선언적 Saga 정의, 보상 자동 역순 실행, Outbox |
...
| 서비스 | 포트 | 외부 API | 핵심 기능 |
|--------|------|----------|----------|
| **concierge-service** | 8088 | DeepSeek | AI 예약 오케스트레이션 (아래 차트 참조) |
...
graph TB
    ...
    subgraph AGENT["concierge-service :8088"]
    ...
    subgraph SERVICES["연동 서비스 (NATS)"]
        COURSE["club-service<br/>골프장·슬롯 조회"]
        BOOK["booking-service<br/>예약 생성·취소"]
        PAY["billing-service<br/>결제·더치페이"]
...
graph TB
  ...
  subgraph PostgreSQL["PostgreSQL StatefulSet"]
    ...
    SAGA_DB[("marketplace_saga_db<br/>SagaExecutions, SagaSteps<br/>OutboxEvents")]:::db
...
graph TB
  ...
  subgraph GKE["GKE Autopilot (asia-northeast3)"]
    ...
    subgraph AppPods["Application Pods"]
      AAPI["manager-bff<br/>Deployment"]:::bff
      UAPI["consumer-bff<br/>Deployment"]:::bff
      ...
      AGENT["concierge-service<br/>Deployment"]:::service
...
|---------|------|-------------|
| manager-bff | 8080 | Admin BFF |
| consumer-bff | 8080 | User BFF |
...
| billing-service | 8086 | Toss Payments |
| weather-service | 8087 | 기상청 API |
| concierge-service | 8088 | AI Agent (DeepSeek) |
