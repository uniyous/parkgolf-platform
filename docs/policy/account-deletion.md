...
| **billing-service** | Payment | userId, bookingId | 유지 (세무) |
| **billing-service** | Refund | - | 유지 (세무) |
| **billing-service** | BillingKey | userId | 삭제 |
...
flowchart TD
    ...
    O --> P4["billing-service<br/>BillingKey 삭제<br/>(Payment, Refund 유지)"]
...
| **manager-console** | 회원 관리 → 삭제 버튼 | 즉시 삭제 확인 모달 | `iam.users.delete` NATS | ✅ 동작 (유예 없음) |
...
| consumer-bff: 계정 삭제 엔드포인트 | ❌ 미구현 | delete-request, delete-cancel, delete-status |
| manager-bff: 사용자 삭제 | ✅ 구현됨 | `DELETE /api/admin/users/:userId` → `iam.users.delete` |
...
| billing-service: `user.deleted` 구독 | ❌ 미구현 | BillingKey 삭제 필요 |
...
| 2.0 | 2026-02-16 | 현재 코드베이스 기준 전체 업데이트: 데이터 분포 현황 갱신 (UserDevice, CompanyMember, billing-service 추가), 서비스별 익명화 상세 제거, 클라이언트 UI 현황 추가 (Web/iOS/Android), 구현 상태 체크리스트 갱신 | - |
