...
| 단계 | 경로 | Client | user-api (BFF) | agent-service | course-service | booking-service | payment-service |
|:----:|:----:|:------:|:--------------:|:-------------:|:--------------:|:---------------:|:---------------:|
| **1. 검색** | LLM | 텍스트 입력 | 전달 | LLM + 도구 실행 | **조회** | | |
| **2. 골프장 선택** | Direct | 카드 클릭 | 전달 | Direct 핸들러 | **조회** | | |
| **3. 슬롯 선택** | Direct | 카드 클릭 | 전달 | Direct (메모리) | | | |
| **4. 예약 + 결제준비** | Direct | 확인 클릭 | 전달 | **원샷 처리** | | **생성 + Saga** | **prepare** |
| **5. 결제** (카드) | Client | **Toss 위젯** | 전달 | | | 확정 (이벤트) | **confirm** |
| **6. 완료** | Direct | 카드 표시 | 전달 | COMPLETE 생성 | | | |
| **자동환불** (엣지) | 내부 | | | | | **감지 + Outbox** | **cancel + 환불** |

> **범례**
> - **굵은 글씨** = 해당 단계의 핵심 처리 컴포넌트
> - **Direct** = LLM 없이 직접 처리 (~200ms) / **LLM** = DeepSeek 경유 (3~10초)
> - **원샷 처리** (4단계): agent-service가 `booking.create` → Saga 폴링 → `payment.prepare`를 한 요청에서 순차 실행. Client는 `orderId`를 받아 Toss 위젯만 표시
> - **PCI DSS** (5단계): 카드 정보 입력(Toss 위젯)은 규정상 Client에서만 실행 가능
