...
3. [Saga 정의](#3-saga-정의)
4. [라이프사이클](#4-라이프사이클)
5. [결제 실패 처리](#5-결제-실패-처리)
6. [NATS 패턴](#6-nats-패턴)
7. [응답 형식](#7-응답-형식)
8. [알려진 결함 및 작업 계획](#8-알려진-결함-및-작업-계획)
...
### 3.6 PAYMENT_FAILED / PAYMENT_TIMEOUT
...
### 6.1 Saga 트리거 (saga-service Inbound)
...
### 6.3 Step 핸들러 (saga-service Outbound)
...
| 6 | `expireSlotReservedBookings`가 saga 우회 → `slot.release` 미호출 | course-service 슬롯 미복구 | P1 | 🟡 부분 해결 (saga-scheduler 신규 경로는 saga 사용, job-service Cron은 그대로) |
...
| 8 | P1 일부 | saga-scheduler `expireSlotReservedBookings` 추가 (1분 주기), `booking.findExpiredSlotReserved` MessagePattern 추가 | ✅ 완료 |
| 9 | 검증 | 통합 테스트, 클라이언트 3종 동작, DB 정합성 | 미시작 (배포 후 진행) |

### 8.3 잔여 P1 작업

P0 완료 후 별도 작업 항목:
- 기존 `expireSlotReservedBookings` (job-service Cron 직접 호출 경로)도 saga publish 방식으로 일원화 검토
