...
> **연계 문서**: 에이전트 전체 워크플로우 [`agent.md`](./agent.md) · UI SSOT [`agent-ui.md`](./agent-ui.md) · 결제(현장·카드·더치페이) [`agent-pay.md`](./agent-pay.md) · 장기 메모리/userMemory [`agent-memory.md`](./agent-memory.md)
...
| 파일 | 역할 |
|---|---|
| `services/concierge-service/src/booking-agent/service/llm-orchestrator.service.ts` | `processWithLLM` · 컨텍스트 주입 · 도구 루프 · 응답 가드 · `updateSlotsFromToolResults` · `createActionsFromToolResults` · `rememberClubs` |
| `services/concierge-service/src/booking-agent/service/deepseek.service.ts` | `systemPromptTemplate` · `chat` / `continueWithToolResults` (system 프롬프트 자동 prepend, `tool_choice` 제어) |
| `services/concierge-service/src/booking-agent/service/conversation.service.ts` | `getRecentMessages` · `updateSlots` · Redis working memory 영속 |
| `services/concierge-service/src/booking-agent/service/ui-card.helper.ts` | `showSelectMembers` (응답 가드에서 SHOW_SLOTS 대체용) |
| `services/concierge-service/src/booking-agent/service/user-memory.service.ts` | Hermes 5-Layer Memory · semantic profile (`get` / `formatProfile`) |
| `services/concierge-service/src/booking-agent/service/direct-action-handler.service.ts` | 카드 클릭 직접 처리 (본 루프 우회) |
| `services/concierge-service/src/booking-agent/dto/chat.dto.ts` | `ConversationContext` · `BookingSlots` 타입 |
