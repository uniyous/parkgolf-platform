...
> 최종 수정: 2026-05-31
> **연계 문서**: 에이전트 전체 플로우 [`agent.md`](./agent.md) · UI(카드·시각흐름·컴포넌트) [`agent-ui.md`](./agent-ui.md) · 컨텍스트 조립 [`agent-context.md`](./agent-context.md) · saga 보상/결제실패 [`saga.md`](./saga.md) · 예약/정산 상태 [`booking.md`](./booking.md)
...
> 슬롯 카드는 `groupMode`(멤버 2명 이상)일 때만 더치페이 옵션을 노출한다 → [`agent-ui.md §5`](./agent-ui.md).
>
...
- 카드 형태(SHOW_PAYMENT, TEAM_COMPLETE) → [`agent-ui.md §4·§5`](./agent-ui.md)
...
에이전트는 **카드 생성·브로드캐스트·정산 완료 판정**을 담당한다. (카드 형태는 [agent-ui.md](./agent-ui.md), 실시간 전달 인프라는 [agent.md §9](./agent.md))
...
관련 카드: `SETTLEMENT_STATUS`, `TEAM_COMPLETE` → [agent-ui.md §4](./agent-ui.md)
관련 요청 필드: `splitPaymentComplete`, `splitOrderId`, `sendReminder` → [agent-ui.md §6](./agent-ui.md)
...
```typescript
// concierge-service → billing-service
...
```mermaid
flowchart TD
    Start([MyBookingsPage<br/>'내 자리 취소' 클릭]) --> API["consumer-bff<br/>DELETE /bookings/:id/participant<br/>JWT userId == participant.userId 검증"]
...
| `payment.refundSplit` | PAYMENT_SERVICE | billing-service | PaymentSplit orderId 단위 Toss 환불 |
...
| `agent-pay.md §10` / `agent-ui.md SHOW_PAYMENT` | 3분 | 현재 값으로 추정 |
