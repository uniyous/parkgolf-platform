...
> 최종 수정: 2026-05-31
> **연계 문서**: 에이전트 전체 플로우 [`agent.md`](./agent.md) · 컨텍스트 조립 [`agent-context.md`](./agent-context.md) · 결제(현장·카드·더치페이) [`agent-pay.md`](./agent-pay.md) · 부킹 상태/멱등성 [`booking.md`](./booking.md)
...
| 영역 | 위치 |
| -- | -- |
| 카드 push (액션 조립) | `concierge-service/src/booking-agent/service/booking-agent.service.ts` |
| 카드 데이터 생성 | `concierge-service/src/booking-agent/service/ui-card.helper.ts` |
| 텍스트 중복 금지 규칙 | `concierge-service/src/booking-agent/service/deepseek.service.ts` (system prompt) |
| ActionType 정의 | `concierge-service/src/booking-agent/dto/chat.dto.ts` |
