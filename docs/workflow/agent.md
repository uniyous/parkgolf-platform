...
> 최종 수정: 2026-05-31
> **연계 문서**: UI(카드·시각흐름·컴포넌트) [`agent-ui.md`](./agent-ui.md) · 결제(현장·카드·더치페이) [`agent-pay.md`](./agent-pay.md) · 컨텍스트 조립 [`agent-context.md`](./agent-context.md) · 메모리/multi-pod [`agent-memory.md`](./agent-memory.md) · 예약 트랜잭션 [`saga.md`](./saga.md) · 예약/정산 상태 [`booking.md`](./booking.md)
>
...
graph TB
    ...
    subgraph BFF
        API["consumer-bff<br/>POST /chat/rooms/:id/agent"]
    end

    subgraph Agent["concierge-service"]
...
대화 컨텍스트를 Redis working memory에 저장. `userId + conversationId` 기반. 멀티 파드 공유 → 어느 파드가 받아도 동일 컨텍스트 복원([`agent-memory.md`](./agent-memory.md)).
...
채팅방 진입 → 메시지 로드
  ...
  └─ 그 외 → AI 모드 자동 ON → concierge-service가 캐시에서 컨텍스트 복원
```
...
UI 전반(카드 종류·분류·데이터·인터랙션·시각 흐름·컴포넌트 props·구현 위치)은 [`agent-ui.md`](./agent-ui.md)에 단일 출처(SSOT)로 분리한다.
...
> **카드/텍스트 비중복 원칙 (UNI-26)**: 카드가 표시하는 데이터(클럽 목록·슬롯 시간·날씨 등)는 텍스트로 다시 나열하지 않는다. 카드는 **결과 시각화** 또는 **액션 유도**일 때만. → [`agent-ui.md`](./agent-ui.md)

---

## 8. 결제

3가지 결제방법(현장·카드·더치페이)의 분기·플로우·책임 분담·타임라인·검증은 [`agent-pay.md`](./agent-pay.md)에 단일 출처(SSOT)로 분리한다.

분기 진입점은 `handleDirectBooking`(슬롯 카드에서 고른 결제수단을 동반해 진입). `booking.create` saga 성공 후 분기.

| 결제방법 | 조건 | 분기 후 | 상세 |
|---------|------|---------|------|
| 현장결제 (`onsite`) | 항상 가능 | 즉시 TEAM_COMPLETE | [agent-pay.md §2](./agent-pay.md) |
| 카드결제 (`card`) | 항상 가능 | SHOW_PAYMENT(참여자명 포함) → Toss → TEAM_COMPLETE | [agent-pay.md §3](./agent-pay.md) |
| 더치페이 (`dutchpay`) | 멤버 2명 이상 | SETTLEMENT_STATUS → 전원 결제 → TEAM_COMPLETE | [agent-pay.md §4~§10](./agent-pay.md) |
...
concierge-service 또는 booking-service가 참여자에게 직접 메시지를 보내야 할 때 사용하는 패턴.

```
concierge-service / booking-service
  │
...
### 11.1 Inbound (concierge-service 수신)
...
| `payment.prepare` | PAYMENT_SERVICE | billing-service | 카드결제 준비 |
| `payment.splitPrepare` | PAYMENT_SERVICE | billing-service | 더치페이 준비 (N명 orderId 발급) |
| `payment.splitStatus` | PAYMENT_SERVICE | billing-service | 분할결제 상태 조회 |
...
concierge-service는 7개의 Named NATS Client를 사용:
...
| `PAYMENT_SERVICE` | billing-service |
...
진행자 화면 단계별 카드 UI 레이아웃(ASCII) → [`agent-ui.md §7`](./agent-ui.md)
...
컴포넌트 매핑·Props 인터페이스(SelectMembersCard / SettlementStatusCard / TeamCompleteCard 등) → [`agent-ui.md §8`](./agent-ui.md)
...
| 서비스 | 경로 |
|--------|------|
| concierge-service | `services/concierge-service/src/booking-agent/` |
...
| billing-service | `services/billing-service/src/payment/service/payment-split.service.ts` |

프론트엔드 카드/버블/VM/페이지 경로 → [`agent-ui.md §9`](./agent-ui.md)

---

## 14. 재개(Resume) — 부수효과 멱등 & Turn Journal (UNI-29 / UNI-42)

> 결정/비결정 경계를 코드로 강제하고, 중단된 턴(크래시·타임아웃·재시도·멀티파드 재라우팅)을 **부수효과 재실행 없이** 이어붙인다. 메모리 계층 설계는 [`agent-memory.md`](./agent-memory.md), 결제 스텝 상세는 [`agent-pay.md`](./agent-pay.md).
