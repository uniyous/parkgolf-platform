...
> **연계 문서**: 인증 기반 주요 흐름 — 채팅/친구 [`chat.md`](./chat.md) · 예약 [`booking.md`](./booking.md) · AI 예약 [`agent.md`](./agent.md)
...
graph TD
    ...
    subgraph BFF Layer
        UserAPI["consumer-bff (BFF)<br/>JWT 검증 (Passport)"]
        AdminAPI["manager-bff (BFF)<br/>NATS 위임 (토큰 전달)"]
...
| `consumer-bff` | 사용자 BFF, JWT 로컬 검증 (Passport), NATS 중계 | 로컬 검증 |
| `manager-bff` | 관리자 BFF, 토큰을 iam-service에 위임 | 없음 (NATS 위임) |
...
| consumer-bff (JwtModule) | `JWT_SECRET` | **필수** (`getOrThrow`) | `auth.module.ts:18` |
| consumer-bff (Passport) | `JWT_SECRET` | **필수** (`getOrThrow`) | `jwt.strategy.ts:11` |
| chat-gateway | `JWT_SECRET` | **필수** (`getOrThrow`) | `auth.module.ts:11` |
| manager-bff | - | - | JWT 모듈 미사용 (NATS 위임) |
...
sequenceDiagram
    ...
    participant BFF as consumer-bff (BFF)
...
**관련 코드:**
- `services/consumer-bff/src/auth/auth.controller.ts:50` — POST `/api/user/iam/login`
- `services/consumer-bff/src/auth/auth.service.ts:78-106` — NATS 전송
...
sequenceDiagram
    participant C as Client
    participant BFF as manager-bff (BFF)
    participant IAM as iam-service
...
**관련 코드:**
- `services/manager-bff/src/auth/auth.controller.ts:35-93` — 로그인 + 역할 검증
...
sequenceDiagram
    participant C as Client
    participant BFF as BFF (consumer-bff / manager-bff)
    participant IAM as iam-service
...
- **사용자 로그아웃:** `consumer-bff` → NATS `iam.auth.user.logout` → DB 삭제
- **관리자 로그아웃:** `manager-bff` → NATS `iam.auth.admin.logout` → DB 삭제

**관련 코드:**
- `services/consumer-bff/src/auth/auth.service.ts` — NATS 호출
- `services/manager-bff/src/auth/auth.controller.ts` — `POST /api/admin/iam/logout`
...
**관련 코드:**
- `services/consumer-bff/src/auth/strategies/jwt.strategy.ts:6-22` — Passport JWT 전략
...
| S4 | 관리자 역할 검증 | manager-bff에서 `hasAdminRole()` 으로 관리자 엔드포인트 보호 |
...
| S8 | Passport 만료 검증 | consumer-bff에서 `ignoreExpiration: false` 설정 |
...
| M3 | **패스워드 검증 규칙 미적용** | **해결** | 공통 상수 `PASSWORD_REGEX` 생성 (영문+숫자+특수문자, 8~128자). `create-user.dto`, `create-admin.dto`에 `@Matches`, `@MaxLength` 적용. manager-bff DTO MinLength 6→8 상향 |
...
| # | 작업 | 상태 | 변경 파일 |
|---|------|------|----------|
| 1-1 | JWT 시크릿 폴백 제거, 환경변수 필수화 | **완료** | `consumer-bff/auth.module.ts`, `consumer-bff/jwt.strategy.ts`, `chat-gateway/auth.module.ts`, `iam-service/auth.module.ts` |
| 1-2 | Access/Refresh 시크릿 분리 | **완료** | `iam-service/auth.service.ts` — `JWT_REFRESH_SECRET` 환경변수로 Refresh Token 별도 서명 |
| 1-3 | Refresh Token DB 저장 + 무효화 | **완료** | `iam-service/auth.service.ts`, `iam-service/auth-nats.controller.ts`, `consumer-bff/auth.service.ts`, `manager-bff/auth.controller.ts`, `manager-bff/auth.service.ts` |
...
| 2-2 | Rate Limiting 적용 | **완료** | `consumer-bff/app.module.ts`, `manager-bff/app.module.ts` — `@nestjs/throttler` 글로벌 60req/60s, 인증 5req/60s |
| 2-3 | WebSocket 주기적 토큰 만료 검증 | **완료** | `chat-gateway/chat.gateway.ts` — 60초 간격, `token_expiring`(5분 전), `token_expired` 이벤트. 웹 클라이언트 리스너 추가 |
| 2-4 | CORS 와일드카드 패턴 제거 | **완료** | `consumer-bff/main.ts`, `manager-bff/main.ts`, `chat-gateway/main.ts`, `chat-gateway/chat.gateway.ts`, `chat-service/main.ts` — `CORS_ALLOWED_ORIGINS` 환경변수 지원 |
...
| 3-3 | 패스워드 정책 통일 | **완료** | `iam-service/common/constants/password.constants.ts` 생성. `create-user.dto`, `create-admin.dto`, manager-bff DTOs 강화 |
...
| `CORS_ALLOWED_ORIGINS` | consumer-bff, manager-bff, chat-gateway, chat-service | 선택 | 쉼표 구분 CORS 허용 목록. 미설정 시 기본 목록 사용 |
...
| `services/consumer-bff/src/auth/auth.controller.ts` | 사용자 BFF 엔드포인트 (login, register, refresh, logout) |
| `services/consumer-bff/src/auth/auth.service.ts` | 사용자 BFF 서비스 (NATS 중계) |
| `services/consumer-bff/src/auth/auth.module.ts` | consumer-bff JWT 모듈 설정 |
| `services/consumer-bff/src/auth/strategies/jwt.strategy.ts` | Passport JWT 전략 (토큰 검증) |
| `services/consumer-bff/src/main.ts` | CORS 설정 |
| `services/manager-bff/src/auth/auth.controller.ts` | 관리자 BFF 엔드포인트 |
| `services/manager-bff/src/auth/auth.service.ts` | 관리자 BFF 서비스 (NATS 중계) |
