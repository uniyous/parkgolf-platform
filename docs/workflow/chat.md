...
> **연계 문서**: AI 모드·정산 카드 [`agent.md`](./agent.md) · 결제(더치페이 broadcast 포함) [`agent-pay.md`](./agent-pay.md) · 알림 [`notification.md`](./notification.md) · 인증/친구 [`auth.md`](./auth.md)
...
flowchart LR
    ...
    ADAPTER <-->|cross-pod broadcast| NATS_EXT{NATS}
    NATS_SVC -->|JetStream + RPC| NATS_EXT
```
...
concierge-service, booking-service 등이 `chat.message.room` 이벤트를 발행하면 chat-gateway가 수신하여 Socket.IO로 전달합니다.
...
| consumer-bff | 1 | 50m / 200m | 96Mi / 256Mi | 없음 |
