...
> **연계 문서**: 예약 알림 [`booking.md`](./booking.md) · 결제(더치페이 결제요청 push 포함) [`agent-pay.md`](./agent-pay.md) · 채팅 메시지 [`chat.md`](./chat.md)
...
6. [알림 자동 해제 정책](#6-알림-자동-해제-정책)
7. [알림 타입 및 템플릿](#7-알림-타입-및-템플릿)
8. [API 엔드포인트](#8-api-엔드포인트)
9. [데이터베이스 스키마](#9-데이터베이스-스키마)
10. [클라이언트 구현](#10-클라이언트-구현)
11. [실시간 알림 (WebSocket)](#11-실시간-알림-websocket)
12. [설정 및 환경변수](#12-설정-및-환경변수)
13. [부록: iOS Push 알림 구현 가이드](#부록-ios-push-알림-구현-가이드)
...
sequenceDiagram
    ...
    participant UA as consumer-bff
...
flowchart LR
    ...
    C3 --> |retryCount >= 3| DLQ
    C5 --> |30일 이상| DELETE
    C6 --> |타입별 TTL 초과| CLEANUP[READ 처리 / 삭제]
```
...
사용자 행동이나 시간 경과에 따라 알림을 자동으로 READ 처리하거나 삭제하는 정책입니다.

### 6.1 개요

| 해제 방식 | 트리거 | 대상 알림 | 처리 |
|-----------|--------|----------|------|
| **이벤트 기반** | 사용자 행동 | 특정 알림 | READ 처리 |
| **TTL 만료** | 크론잡 (매일 02:00) | 만료된 알림 | READ 처리 / 삭제 |

### 6.2 이벤트 기반 자동 해제

#### NATS 이벤트: `notification.dismiss`

타입 + JSON data 조건으로 미읽은 알림을 일괄 READ 처리합니다.

```typescript
// 이벤트 페이로드
interface NotificationDismissEvent {
  userId: string;
  type: string;           // NotificationType (예: 'FRIEND_REQUEST', 'CHAT_MESSAGE')
  dataFilter?: Record<string, any>;  // JSON 필드 조건 (단일 키)
}
```
...
```mermaid
flowchart TB
    subgraph FriendFlow["친구 요청 알림 해제"]
        FR1[User B가 친구 요청 수신] --> FR2[FRIEND_REQUEST 알림 생성]
        FR2 --> FR3{User B의 행동}
        FR3 -->|수락| FR4[iam-service: acceptFriendRequest]
        FR3 -->|거절| FR5[iam-service: rejectFriendRequest]
        FR4 & FR5 --> FR6[notification.dismiss 발행<br/>type: FRIEND_REQUEST<br/>dataFilter: requestId]
        FR6 --> FR7[FRIEND_REQUEST 알림 READ 처리]
    end

    subgraph ChatFlow["채팅 메시지 알림 해제"]
        CM1[User A가 오프라인 상태에서<br/>채팅 메시지 수신] --> CM2[CHAT_MESSAGE 알림 생성]
        CM2 --> CM3[User A가 채팅방 입장<br/>join_room 이벤트]
        CM3 --> CM4[notification.dismiss 발행<br/>type: CHAT_MESSAGE<br/>dataFilter: chatRoomId]
        CM4 --> CM5[해당 채팅방 알림 READ 처리]
    end
```
...
sequenceDiagram
    ...
    Note over B,NS: 친구 요청 수락 시
    B->>UA: POST /friends/accept {requestId}
    UA->>IAM: NATS: friends.accept
    IAM->>IAM: 친구 관계 생성
    IAM-->>NS: friend.accepted (Event)
    IAM-->>NS: notification.dismiss (Event)<br/>{userId: B, type: FRIEND_REQUEST,<br/>dataFilter: {requestId}}
    NS->>NS: FRIEND_REQUEST 알림 READ 처리

    Note over B,NS: 친구 요청 거절 시
    B->>UA: POST /friends/reject {requestId}
    UA->>IAM: NATS: friends.reject
    IAM->>IAM: 요청 상태 → REJECTED
    IAM-->>NS: notification.dismiss (Event)<br/>{userId: B, type: FRIEND_REQUEST,<br/>dataFilter: {requestId}}
    NS->>NS: FRIEND_REQUEST 알림 READ 처리
```

### 6.3 채팅 메시지 알림 (오프라인 사용자)

채팅 메시지 발송 시, 해당 채팅방에 접속하지 않은 참여자에게 Push 알림을 보냅니다.
...
sequenceDiagram
    ...
    participant CS as chat-service
    participant NS as notify-service
    participant R as Recipient (오프라인)

    S->>CG: WebSocket: send_message
    CG->>CG: Socket.IO 룸 브로드캐스트
    CG->>CS: messages.save (비동기)

    Note over CG: 오프라인 참여자 확인
    CG->>CS: rooms.get {roomId}
    CS-->>CG: {members: [...]}
    CG->>CG: 현재 룸에 접속한 소켓 확인
    CG->>CG: 미접속 멤버 필터링

    loop 오프라인 멤버
        CG-->>NS: chat.message (Event)<br/>{chatRoomId, senderId, senderName,<br/>recipientId, messagePreview}
        NS->>NS: CHAT_MESSAGE 알림 생성
        NS-->>R: Push: "Sender님의 새 메시지"
    end

    Note over R,CG: 나중에 채팅방 입장 시
    R->>CG: WebSocket: join_room
    CG-->>NS: notification.dismiss<br/>{type: CHAT_MESSAGE, dataFilter: {chatRoomId}}
    NS->>NS: 해당 채팅방 알림 READ 처리
```
...
| 상태 | 조건 | 알림 여부 |
|------|------|----------|
| 룸 접속 중 | `roomSubscriptions`에 소켓 ID 존재 | Push 미발송 |
| 오프라인 | `roomSubscriptions`에 소켓 ID 미존재 | Push 발송 |
| 메시지 발신자 | `sender.id === member.userId` | Push 미발송 |

### 6.4 TTL 만료 크론잡

매일 새벽 2시에 실행되며, 타입별 TTL에 따라 오래된 알림을 정리합니다.

#### 타입별 TTL 설정

| 알림 타입 | TTL (일) | 설명 |
|----------|---------|------|
| `CHAT_MESSAGE` | 7 | 채팅 알림은 단기 유효 |
| `FRIEND_REQUEST` | 30 | 친구 요청은 장기 유효 |
| `FRIEND_ACCEPTED` | 14 | 수락 알림은 중기 유효 |
| `BOOKING_CONFIRMED` | 7 | 예약 확정 후 단기 유효 |
| `BOOKING_CANCELLED` | 7 | 예약 취소 후 단기 유효 |
| `PAYMENT_SUCCESS` | 30 | 결제 내역은 장기 유효 |
| `PAYMENT_FAILED` | 30 | 결제 실패도 장기 유효 |
| `SYSTEM_ALERT` | 30 | 시스템 알림은 장기 유효 |

#### 크론잡 처리 플로우

```mermaid
flowchart TB
    CRON["⏰ Cron: 매일 02:00"] --> LOOP["타입별 TTL 순회"]

    LOOP --> |각 타입| CUTOFF["cutoff = now - TTL"]

    CUTOFF --> DEL["deleteExpired<br/>createdAt < cutoff AND status = READ"]
    CUTOFF --> MARK["markExpiredAsRead<br/>createdAt < cutoff AND readAt = null"]

    DEL --> |삭제 수| TOTAL["합산 통계"]
    MARK --> |READ 처리 수| TOTAL

    TOTAL --> |deleted > 0 or marked > 0| LOG["📊 로그 출력<br/>deleted=N, marked_read=M"]
```
...
stateDiagram-v2
    ...
    SENT --> READ: 사용자가 읽음
    SENT --> READ: 이벤트 기반 자동 해제<br/>(notification.dismiss)
    SENT --> READ: TTL 만료 자동 READ 처리

    FAILED --> PENDING: 재시도 (retryCount < 3)
    FAILED --> DLQ: 영구 실패 (retryCount >= 3)

    READ --> DELETED: TTL 만료 삭제<br/>(크론잡)
...
### 6.5 NATS 이벤트 플로우 (자동 해제)

```mermaid
flowchart LR
    subgraph Publishers["이벤트 발행자"]
        IAM[iam-service<br/>친구 수락/거절]
        CG[chat-gateway<br/>방 입장]
    end

    subgraph Event["NATS Event"]
        E1[notification.dismiss]
    end

    subgraph Handler["이벤트 처리"]
        NS[notify-service<br/>NotificationNatsController]
        SVC[NotificationService<br/>dismissByType]
    end

    IAM -->|FRIEND_REQUEST<br/>+ requestId| E1
    CG -->|CHAT_MESSAGE<br/>+ chatRoomId| E1
    E1 --> NS
    NS --> SVC

    SVC -->|Prisma JSON path 필터| DB[(PostgreSQL<br/>updateMany)]
```
...
| 서비스 | 파일 | 역할 |
|--------|------|------|
| notify-service | `notification/service/notification.service.ts` | `dismissByType()`, `deleteExpired()`, `markExpiredAsRead()` |
| notify-service | `notification/notification-nats.controller.ts` | `notification.dismiss` EventPattern 핸들러 |
| notify-service | `notification/service/scheduler.service.ts` | TTL 만료 크론잡 (`cleanupExpiredNotifications`) |
| iam-service | `friend/friend.service.ts` | 수락/거절 시 `notification.dismiss` emit |
| chat-gateway | `nats/nats.module.ts` | `NOTIFY_SERVICE` ClientProxy 등록 |
| chat-gateway | `nats/nats.service.ts` | `emitChatMessageNotification()`, `emitNotificationDismiss()` |
| chat-gateway | `gateway/chat.gateway.ts` | 메시지 발송 시 오프라인 알림 + 방 입장 시 알림 해제 |
...
### 12.2 chat-gateway 환경변수 (WebSocket)
