...
>
> **연계 문서**: 가맹점이 계약 시 제공해야 하는 표준 데이터 명세는 [`docs/policy/partner-ris.md`](../policy/partner-ris.md) 참조. 예약 연동은 [`booking.md`](./booking.md) · 예약 트랜잭션 [`saga.md`](./saga.md).
...
> - **RIS 표준 명세** (partner-ris.md) 회신으로 마법사가 80% 자동 채움 — probe/LLM은 미준수 가맹점 fallback
...
flowchart LR
    ...
    subgraph WIZARD["🪄 등록 마법사 (marketplace-console)"]
...
flowchart TB
    subgraph UI["🖥️ UI"]
        PW["marketplace-console<br/>/partners/wizard"]
        PR["marketplace-console<br/>/partners/:id/review"]
        AC["manager-console<br/>/clubs/:id 파트너 탭"]
    end

    subgraph API["🌐 manager-bff (BFF)"]
        E1["/admin/partners/*"]
...
- 가맹점에 [`docs/policy/partner-ris.md`](../policy/partner-ris.md) + [`docs/partner/RIS_TEMPLATE.yaml`](../partner/RIS_TEMPLATE.yaml) 전달
...
sequenceDiagram
    ...
    participant W as marketplace-console
    participant API as manager-bff
...
sequenceDiagram
    ...
    box rgb(232, 245, 233) 🖥️ System (UI/API/Service)
    participant Adm as manager-console
    participant API as manager-bff
    participant PS as partner-service
...
%%{init: {'theme':'base','themeVariables':{'fontSize':'13px'}}}%%
flowchart TD
    Start([파트너 계약 체결]) --> Wizard["marketplace-console<br/>/partners/wizard"]
    ...
    TR -->|PASS| FConf["manager-console<br/>/clubs/:id 파트너 탭<br/>동의 카드"]
    TR -->|FAIL| ReviewD
...
### 7.1 marketplace-console 신규 라우트
...
flowchart TD
    ...
    Conf -->|0.5~0.8| LLM["LLM 검증<br/>concierge-service DeepSeek"]
...
```
가맹점 운영자: manager-console에서 예약 #135 상세
→ "외부 ERP에서 실시간 조회" 버튼
...
| 파일 | 용도 |
|------|------|
| `docs/policy/partner-ris.md` | **가맹점 회신 표준 명세** (RIS_HANDOFF 단계의 입력) |
...
| `services/concierge-service/src/booking-agent/service/tool-executor.service.ts` | LLM 호출 패턴 참조 (fallback 매핑 추천용) |
| `apps/manager-console/src/components/features/club/PartnerStatusPanel.tsx` | ACTIVE 후 가맹점 view 재사용 + Realtime 조회 버튼 |
| `apps/marketplace-console/src/pages/franchise/FranchiseClubsPage.tsx` | 마법사 진입점 통합 위치 |
...
가맹점 운영자가 manager-console에서 예약/결제 상세를 볼 때 **stale 데이터 방지**.

### 17.1 3-Layer 구조

```mermaid
flowchart LR
    UI["manager-console<br/>예약 상세"] --> L1{"L1: Redis Cache<br/>TTL 30s (결제 10s)"}
    L1 -->|hit| Resp["응답"]
...
| `apps/manager-console/src/components/features/club/PartnerStatusPanel.tsx` | 실시간 조회 버튼 + 응답 표시 |
...
| 2026-05-16 | 0.3 | RIS_HANDOFF 단계 추가 · §5.0 신규 · §5.1을 fallback으로 격하 · §16 PartnerAdapterSkill · §17 Realtime Booking Proxy · §12 NATS 패턴 보강 · partner-ris.md 연계 |
