...
> 버전: 3.5
> 최종 수정: 2026-05-13
> **연계 문서**: AI 예약 흐름 [`agent.md`](./agent.md) · 결제(현장·카드·더치페이) end-to-end [`agent-pay.md`](./agent-pay.md) · 예약/정산 상태 [`booking.md`](./booking.md)
...
flowchart TB
    subgraph Triggers["📨 Saga 트리거"]
        UA["👥 consumer-bff / manager-bff / concierge-service"]
        PS_OBX["📤 billing-service<br/>(outbox processor)"]
    ...
    subgraph StepHandlers["⚙️ Step Handlers"]
        ...
        CS["📅 club-service<br/>slot.reserve / slot.release"]
        PAY["💳 billing-service<br/>payment.cancelByBookingId<br/>payment.markAborted"]
...
flowchart TD
    ...
    Branch -->|"😀 4명 모두 결제 완료"| AllPaid["✅ booking.paymentConfirmed<br/>(billing-service outbox)"]
    ...
    Step2 --> Step3["3️⃣ RELEASE_SLOT<br/>club-service 슬롯 복구"]
...
concierge-service는 ①~③의 UI 카드를 발행하는 **드라이버**일 뿐,
**상태 변화의 권위(source of truth)는 saga + booking/billing-service**.

#### 6.6.2 변경 전 vs 변경 후

| 단계 | 현재 (변경 전) | 제안 (변경 후) |
|------|----------------|----------------|
| splitPrepare 호출 | concierge-service의 tool-executor가 NATS 직접 호출 | `CREATE_BOOKING` saga의 신규 step에서 처리 |
| settlement card broadcast | concierge-service가 직접 chat-gateway emit | saga 신규 step에서 처리 (실패 시 saga 보상) |
...
flowchart TD
    ...
    S1 --> S4["S4. RESERVE_SLOT<br/>club-service 슬롯 확보"]
...
| concierge-service `tool-executor` | `payment.splitPrepare` 직접 호출 **제거** (saga로 위임) |
| concierge-service `tool-executor` | settlement card chat broadcast 직접 호출 **제거** (saga로 위임) |
...
| Phase 4 | concierge-service의 splitPrepare/broadcast 직접 호출 제거 | 낮음 (saga가 책임 보유 후 정리) | Phase 3 후 |
...
### 6.8 billing-service cron 제거 및 pg-boss 전환 (2026-05-05)
...
sequenceDiagram
    ...
    participant P as billing-service
...
| 파일 | 변경 내용 |
|---|---|
| `billing-service/src/payment/service/payment.service.ts` | confirmPayment에 paymentKey 사전 저장 + reconcile 잡 schedule + 네트워크 불명 catch 분기. `reconcilePayment` / `scheduleReconcile` / `isNetworkUncertainError` / `triggerOutboxJob` / `insertOutboxEvent` 추가. `markPaymentAborted` 트랜잭션 commit 후 트리거 패턴으로 리팩토링 |
| `billing-service/src/payment/service/payment-reconcile.service.ts` | 신규 — pg-boss worker. `paymentService.reconcilePayment` 위임 |
| `billing-service/src/payment/service/outbox-processor.service.ts` | pg-boss worker로 변경 (cron 제거). `OUTBOX_QUEUE = 'payment-outbox-publish'` |
...
| 패턴 | 발신 | 비고 |
|------|------|------|
| `saga.booking.create` | consumer-bff, concierge-service | 동기 응답 |
| `saga.booking.cancel` | consumer-bff, manager-bff | 동기 응답 |
| `saga.booking.adminRefund` | manager-bff | 동기 응답 |
| `booking.paymentConfirmed` | billing-service (outbox) | 비동기 |
| `booking.paymentFailed` 신규 | billing-service (outbox) | 비동기 |
...
| `slot.reserve` / `slot.release` | club-service |
| `payment.cancelByBookingId` | billing-service |
| `payment.refundPaidSplits` | billing-service (분할결제 일괄 환불) |
...
| 6 | `expireSlotReservedBookings`가 saga 우회 → `slot.release` 미호출 | club-service 슬롯 미복구 | P1 | ✅ 해결 (job-service `booking-payment-timeout` cron 제거 + booking-service 우회 경로 제거) |
...
| 10 | billing-service outbox processor 동작 검증 필요 | confirmed 이벤트 누락 가능 | P1 | ✅ 검증 (pg-boss 즉시 트리거로 전환, 6.7) |
...
| 14 | billing-service raw outbox INSERT 2곳 (pg-boss 미트리거) | 영구 PENDING 누락 (가상계좌 입금 / handleAbort) | P1 | ✅ 해결 (createOutboxEvent / insertOutboxEvent 통일, 6.7) |
| 15 | billing-service cron (saga-scheduler) 잔존 | 멀티 pod 운영 시 중복 실행 위험 | P1 | ✅ 해결 (cron 제거, pg-boss로 통일, 6.7) |
