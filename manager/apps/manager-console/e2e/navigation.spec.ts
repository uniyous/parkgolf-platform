...
test.describe('사이드바 네비게이션 테스트', () => {
  ...
  test('사이드바 브랜드 표시', async ({ page }) => {
    // 브랜드 텍스트 확인
    await expect(page.getByText('파크골프 관리')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('회사 관리 페이지 이동', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/.*companies/, { timeout: 30000 });

    // 페이지 콘텐츠 확인
    await expect(page.getByText(/회사|Company/).first()).toBeVisible({ timeout: 30000 });
  });

  test('골프장 관리 페이지 이동', async ({ page }) => {
    // 골프장 관리 링크 클릭
    await page.getByRole('link', { name: /골프장 관리/ }).click();
    await expect(page).toHaveURL(/.*clubs/, { timeout: 30000 });

    // 페이지 콘텐츠 확인
    await expect(page.getByText(/골프장|클럽/).first()).toBeVisible({ timeout: 30000 });
  });
  ...
  test('라운드 관리 페이지 이동', async ({ page }) => {
    ...
    // 페이지 콘텐츠 확인
    await expect(page.getByText(/라운드|게임/).first()).toBeVisible({ timeout: 30000 });
  });
  ...
  test('예약 현황 페이지 이동', async ({ page }) => {
    ...
    // 페이지 콘텐츠 확인
    await expect(page.getByText(/예약/).first()).toBeVisible({ timeout: 30000 });
  });
  ...
  test('사용자 관리 페이지 이동', async ({ page }) => {
    ...
    // 페이지 콘텐츠 확인
    await expect(page.getByText(/사용자|회원/).first()).toBeVisible({ timeout: 30000 });
  });

  test('관리자 관리 페이지 이동', async ({ page }) => {
    // 관리자 관리 링크 클릭
    await page.getByRole('link', { name: /관리자 관리/ }).click();
    await expect(page).toHaveURL(/.*admin-management/, { timeout: 30000 });

    // 페이지 콘텐츠 확인
    await expect(page.getByText(/관리자/).first()).toBeVisible({ timeout: 30000 });
  });

  test('역할 및 권한 관리 페이지 이동', async ({ page }) => {
    // 역할 및 권한 관리 링크 클릭
    await page.getByRole('link', { name: /역할 및 권한 관리/ }).click();
    await expect(page).toHaveURL(/.*roles/, { timeout: 30000 });

    // 페이지 콘텐츠 확인
    await expect(page.getByText(/역할|권한/).first()).toBeVisible({ timeout: 30000 });
  });
});
...
test.describe('헤더 테스트', () => {
  ...
  test('사용자 메뉴 표시', async ({ page }) => {
    // 페이지가 로드되면 통과 (beforeEach에서 이미 대시보드 헤더 확인)
