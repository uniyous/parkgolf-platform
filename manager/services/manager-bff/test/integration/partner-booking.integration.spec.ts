/**
 * Partner Booking Integration Tests (E2E via manager-bff)
 *
 * 테스트 대상: manager-bff → NATS → partner-service 전체 흐름
 *
 * 시나리오:
 ...
 *
 * 사전 조건:
 * - manager-bff, partner-service, iam-service가 NATS에 연결되어 있어야 함
...
describe('Partner Booking Integration Tests', () => {
  ...
  describe('0. 테스트 환경 확인', () => {
    it('TC-000: manager-bff Health Check', async () => {
      try {
 ...
 *
 * 1. 파트너 설정 생성
 * curl -s -X POST "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/partners" \
 ...
 *
 * 2. 파트너 설정 목록
 * curl -s "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/partners?page=1&limit=10" \
 *   -H "Authorization: Bearer <TOKEN>" | jq .
 *
 * 3. 게임 매핑 생성
 * curl -s -X POST "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/partners/1/game-mappings" \
 *   -H "Content-Type: application/json" \
 *   -H "Authorization: Bearer <TOKEN>" \
 *   -d '{"externalCourseName":"A+B코스","internalGameId":1}' | jq .
 *
 * 4. 동기화 이력
 * curl -s "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/partners/my/club/1/sync-logs" \
 *   -H "Authorization: Bearer <TOKEN>" | jq .
 *
 * 5. 웹훅 테스트
