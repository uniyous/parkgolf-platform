...
export class StepExecutorService implements StepExecutorPort {
  ...
  constructor(
    @Inject('FRONTDESK_SERVICE') private readonly frontdeskClient: ClientProxy,
  ...
  private getClient(targetService: string): ClientProxy {
    switch (targetService) {
      case 'FRONTDESK_SERVICE': return this.frontdeskClient;
      case 'CLUB_SERVICE': return this.clubClient;
      ...
      default:
        this.logger.warn(`Unknown target service: ${targetService}, falling back to FRONTDESK_SERVICE`);
        return this.frontdeskClient;
    }
  }
}
