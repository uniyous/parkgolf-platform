...
async function bootstrap() {
  ...
  try {
    ...
    const config = new DocumentBuilder()
      .setTitle('Parkgolf Payment Service API')
      .setDescription('현장 수납 기록·환불·일마감')
      .setVersion('1.0')
      .build();
    ...
    logger.log('=== NATS Patterns (payment-service 수납) ===');
    logger.log('  [Saga]  payment.collect / payment.refund  (manager-saga COLLECT_PAYMENT 계약)');
    logger.log('  [Admin] payment.list / payment.get / payment.dailyClose');
  } catch (error) {
