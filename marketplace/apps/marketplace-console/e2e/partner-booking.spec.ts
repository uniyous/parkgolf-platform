...
// ========================================
// 3. 데이터 동기화 (Manual Sync via manager-bff)
// ========================================
test.describe('3. 데이터 동기화', () => {
  ...
  test('TC-304: 예약 매핑 조회 (clubId 기반, manager-console)', async ({ request }) => {
    const token = await getToken(request);
    if (!token || !createdClubId) { test.skip(); return; }
  ...
  // --- marketplace-console 전용 라우트 (partnerId 기반) ---

  test('TC-305: 동기화 이력 조회 (partnerId 기반, marketplace-console)', async ({ request }) => {
    const token = await getToken(request);
    if (!token || !createdPartnerId) { test.skip(); return; }
  ...
  test('TC-306: 수동 동기화 트리거 (partnerId 기반, marketplace-console)', async ({ request }) => {
    test.setTimeout(30000);
  ...
  test('TC-307: 예약 매핑 조회 (partnerId 기반, marketplace-console)', async ({ request }) => {
    const token = await getToken(request);
    if (!token || !createdPartnerId) { test.skip(); return; }
