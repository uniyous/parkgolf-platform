...
class ApiClient {
  ...
  /**
   * 지원 모드에서 선택된 가맹점 ID를 localStorage에서 읽어옴
   * Zustand persist 미들웨어가 'support-storage' 키로 저장
   */
  private getSupportCompanyId(): number | null {
    try {
      const raw = localStorage.getItem('support-storage');
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      const state = parsed?.state;
      if (state?.isSupportMode && state?.selectedCompany?.id) {
  ...
  private async request<T>(
    ...
    const supportCompanyId = this.getSupportCompanyId();
    const headers = {
      'Content-Type': 'application/json',
      ...(token && { Authorization: `Bearer ${token}` }),
      ...(supportCompanyId && { 'X-Company-Id': String(supportCompanyId) }),
