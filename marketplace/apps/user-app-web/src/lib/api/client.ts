...
// 토큰 갱신 엔드포인트
const REFRESH_ENDPOINT = '/api/user/iam/refresh';
  ...
  private async handleErrorResponse(
    ...
    if ('success' in errorData && errorData.error) {
      ...
      errorMessage = getErrorMessage(
        errorCode,
        errorData.message || errorData.error?.message || '오류가 발생했습니다'
      );
      errorDetails = errorData.error?.details;
    }
    ...
    if (response.status === 401) {
      ...
      if (!endpoint.includes(REFRESH_ENDPOINT) && !endpoint.includes('/auth/refresh')) {
