...
interface CourseBulkActionsProps {
  ...
  isLoading: boolean;
  onBulkAction: (action: string, courseIds: number[]) => void;
}
...
export const CourseBulkActions: React.FC<CourseBulkActionsProps> = ({
  ...
  const handleAction = (action: string) => {
    onBulkAction(action, selectedCourses);
  };
    ...
    <div className="bg-white border-l-4 border-green-500 rounded-lg shadow-sm p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <div className="flex items-center">
            ...
            <span className="text-sm font-medium text-gray-900">
              {selectedCount}개 코스가 선택됨
            </span>
          </div>
          
          <div className="text-sm text-gray-500">
            선택된 코스들에 대해 일괄 작업을 수행할 수 있습니다.
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={() => handleAction('update_pricing')}
            disabled={isLoading}
          ...
          <button
            ...
            disabled={isLoading}
            className="inline-flex items-center px-3 py-2 text-sm font-medium rounded-md text-purple-700 bg-purple-100 hover:bg-purple-200 focus:outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mt-3 text-xs text-gray-500">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div>
            <strong>활성화:</strong> 선택된 코스들을 활성 상태로 변경합니다.
          </div>
          <div>
            <strong>점검 모드:</strong> 선택된 코스들을 임시 점검 상태로 설정합니다.
          </div>
          <div>
            <strong>가격 수정:</strong> 선택된 코스들의 가격을 일괄 수정합니다.
          </div>
        </div>
      </div>
