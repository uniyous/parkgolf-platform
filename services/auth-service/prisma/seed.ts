...
async function main() {
  console.log('Starting database seeding with simplified structure (v3)...');

  // 기존 데이터 삭제
  await clearAllData();

  // 1. 권한 마스터 데이터 생성 (18개: 관리자 10개 + 사용자 8개)
  console.log('Creating permission masters (18 total)...');
  const permissions = [
    // 관리자 권한 (10개)
    ...
    // 사용자 권한 (8개 - 단순화됨)
    { code: 'PROFILE', name: '프로필 관리', category: 'USER', level: 'low', description: '프로필 조회 및 수정' },
    { code: 'COURSE_VIEW', name: '코스 조회', category: 'USER', level: 'low', description: '골프장/코스/타임슬롯 조회' },
    { code: 'BOOKING_VIEW', name: '예약 조회', category: 'USER', level: 'low', description: '내 예약 조회 및 이력' },
    { code: 'BOOKING_MANAGE', name: '예약 관리', category: 'USER', level: 'low', description: '예약 생성/수정/취소' },
    { code: 'PAYMENT', name: '결제/환불', category: 'USER', level: 'low', description: '결제 조회 및 환불 요청' },
  ...
  console.log(`Created ${permissions.length} permissions (Admin: 10, User: 8)`);

  // 2. 역할 마스터 데이터 생성 (5+3 = 8개)
  console.log('Creating role masters (8 total)...');
  const roles = [
    // 관리자 역할 (5개)
  ...
  console.log(`Created ${roles.length} roles (Admin: 5, User: 3)`);
  ...
  for (const adminData of testAdmins) {
    ...
    console.log(`Created admin: ${admin.email} (${admin.name} - ${admin.roleCode}) → [${rolePerms.join(', ')}]`);
  }
  ...
  for (const userData of testUsers) {
    ...
    console.log(`Created user: ${user.email} (${user.name} - ${user.roleCode}) → [${rolePerms.join(', ')}]`);
  }
  ...
  console.log('========================================');
  console.log('\n권한 구조 요약:');
  console.log('  - 관리자 권한: 10개 (ALL, COMPANIES, COURSES, TIMESLOTS, BOOKINGS, USERS, ADMINS, ANALYTICS, SUPPORT, VIEW)');
  console.log('  - 사용자 권한: 8개 (PROFILE, COURSE_VIEW, BOOKING_VIEW, BOOKING_MANAGE, PAYMENT, PREMIUM_BOOKING, PRIORITY_BOOKING, ADVANCED_SEARCH)');
  console.log('  - 총 18개 권한');
  console.log('\nTest Admin Accounts:');
