...
async function bootstrap() {
  ...
  try {
    logger.log('Starting Job Service...');
    ...
    const config = new DocumentBuilder()
      .setTitle('Parkgolf Job Service API')
      .setDescription('Batch job scheduler service')
      .setVersion('1.0')
      .build();
    ...
    // Start HTTP server first for health check
    ...
    if (process.env.NATS_URL) {
      connectNatsWithRetry(app, process.env.NATS_URL, 'job-service', logger).catch(() => {});
    } else {
