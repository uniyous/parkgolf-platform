 ...
 *
 * 테스트 대상: manager-bff와 auth-service 간의 Admin 인증/권한 처리 흐름
 *
 * 사전 조건:
 * - auth-service가 NATS에 연결되어 있어야 함
 * - manager-bff가 NATS에 연결되어 있어야 함
 * - 데이터베이스가 초기화되어 있어야 함
 */
...
describe('Admin Authentication Integration Tests', () => {
  ...
  const baseUrl = process.env.ADMIN_API_URL || 'https://manager-bff-dev-iihuzmuufa-du.a.run.app';
  ...
  describe('2. Admin Login Flow', () => {
     ...
     *
     * 시나리오: consumer-bff로 등록한 일반 사용자가 manager-bff로 로그인 시도
    ...
    it('TC-007: Should fail to login as admin with regular user account', async () => {
      ...
      const response = await request(baseUrl)
        .post('/api/admin/auth/login')
        .send({
          email: 'testuser9999@example.com', // consumer-bff로 등록된 일반 사용자
          password: 'test1234',
        })
        .expect('Content-Type', /json/);
 ...
 *
 * 1. Health Check
 * curl -s https://manager-bff-dev-iihuzmuufa-du.a.run.app/health | jq .
 *
 * 2. Admin Signup
 * curl -s -X POST "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/auth/signup" \
 *   -H "Content-Type: application/json" \
 *   -d '{"username":"testadmin","email":"testadmin@example.com","password":"Admin1234","name":"Test Admin","role":"ADMIN"}' | jq .
 *
 * 3. Admin Login
 * curl -s -X POST "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/auth/login" \
 *   -H "Content-Type: application/json" \
 *   -d '{"email":"testadmin@example.com","password":"Admin1234"}' | jq .
 *
 * 4. Get Current User (with token)
 * curl -s "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/auth/me" \
 *   -H "Authorization: Bearer <ACCESS_TOKEN>" | jq .
 *
 * 5. Refresh Token
 * curl -s -X POST "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/auth/refresh" \
 *   -H "Content-Type: application/json" \
 *   -d '{"refreshToken":"<REFRESH_TOKEN>"}' | jq .
 *
 * 6. Get Admin List
 * curl -s "https://manager-bff-dev-iihuzmuufa-du.a.run.app/api/admin/admins" \
 *   -H "Authorization: Bearer <ACCESS_TOKEN>" | jq .
 */
