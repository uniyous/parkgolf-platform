...
import { CreateBookingDto, UpdateBookingDto, SearchBookingDto, CancelBookingDto } from './dto/booking.dto';
...
export class BookingController {
  ...
  @ApiResponse({ status: 200, description: '예약 목록을 성공적으로 조회했습니다.' })
  ...
  @ApiResponse({ status: 200, description: '검색 결과를 성공적으로 조회했습니다.' })
  async searchBookings(@Query() searchDto: SearchBookingDto) {
    this.logger.log(`Searching bookings with filters: ${JSON.stringify(searchDto)}`);
  ...
  @ApiResponse({ status: 200, description: '예약이 성공적으로 수정되었습니다.' })
  @ApiResponse({ status: 404, description: '예약을 찾을 수 없습니다.' })
  ...
  @ApiResponse({ status: 200, description: '타임슬롯 가용성을 성공적으로 조회했습니다.' })
