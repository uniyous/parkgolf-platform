...
import { CreateBookingDto, SearchBookingDto, UpdateBookingDto } from './dto/booking.dto';
...
export class BookingService {
  ...
  async createBooking(userId: number, dto: CreateBookingDto) {
    try {
      this.logger.log(`Creating booking for user ${userId}: ${JSON.stringify(dto)}`);
  ...
  async getBookingById(id: number) {
    try {
      ...
      const result = await firstValueFrom(
        this.bookingClient
          .send('booking.findById', { id })
          .pipe(timeout(5000))
      );

      return result;
    } catch (error) {
      this.logger.error(`Failed to get booking by ID: ${error.message}`, error.stack);
      throw new HttpException(
        'Booking not found',
  ...
  async getBookingByNumber(bookingNumber: string) {
    try {
      ...
      this.logger.error(`Failed to get booking by number: ${error.message}`, error.stack);
      throw new HttpException(
        'Booking not found',
        HttpStatus.NOT_FOUND
      );
  ...
  async getBookingsByUserId(userId: number) {
    try {
      ...
      this.logger.error(`Failed to get bookings by user ID: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async searchBookings(searchDto: SearchBookingDto) {
    try {
      ...
      this.logger.error(`Failed to search bookings: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  async getTimeSlotAvailability(courseId: number, date: string) {
    try {
      this.logger.log(`Getting time slot availability for course ${courseId} on ${date}`);
      ...
      this.logger.error(`Failed to get time slot availability: ${error.message}`, error.stack);
      throw new HttpException(
